...
- **Release notes** (standard practice, ported from tkforgeworks/anvil): `scripts/generate-release-notes.js` derives the body from commit subjects since the previous tag — version-bump commits filtered, subjects split into Changes vs Bug Fixes (bug-fix commit subjects must start with `Fix` or `CGUI-N: Fix ...`), `CGUI-*` keys auto-linked via the `JIRA_BASE_URL` repo variable. Stable releases diff against the previous *stable* tag so final notes span all RCs. Write commit subjects knowing they become changelog lines
